function render(canvas, width, height, colors) {
    canvas.width = width;
...
function QOIChunks(input, begin = 0, end = input.length, a_state) {
  const EOI = end - begin; let self;
...
function QOIChunk(input, begin = 0, end = input.length, a_state) {
  const EOI = end - begin; let self;
...
function Chunk(input, begin = 0, end = input.length, a_state, a_tagByte) {
  const EOI = end - begin; let self;
...
export async function drawQOI(canvas, src) {
    const response = await fetch(src);
    const qoiData = QOI(await response.bytes());
    render(canvas, qoiData.width, qoiData.height, qoiData.colors); 
}
