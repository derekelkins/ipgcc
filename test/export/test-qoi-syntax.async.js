...
async function QOI(input, begin = 0, end = input.length) {
  ...
  _ipg_alt: {
    ...
    right = EOI;
    if (left < 0 || right < left || right > EOI) break _ipg_alt;
    nt_QOIChunks_0 = await QOIChunks(input, begin + left, begin + right);
...
async function QOIChunks(input, begin = 0, end = input.length) {
  const EOI = end - begin; let self;
  ...
  _ipg_alt: {
    ...
    while (true) {
      ...
      nt_QOIChunk_0 = await QOIChunk(input, begin + left, begin + right);
...
async function QOIChunk(input, begin = 0, end = input.length) {
  const EOI = end - begin; let self;
  ...
  _ipg_alt: {
    ...
    right = EOI;
    if (left < 0 || right < left || right > EOI) break _ipg_alt;
    nt_Chunk_0 = await Chunk(input, begin + left, begin + right, self.tagByte);
...
async function Chunk(input, begin = 0, end = input.length, a_tagByte) {
  const EOI = end - begin; let self;
