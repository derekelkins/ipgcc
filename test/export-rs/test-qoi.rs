...
fn QOI(input: &[u8], begin: usize, end: usize) -> Option<(usize, usize, QOI)> {
  ...
  '_ipg_alt: {
    ...
    let mut self_state = initState(self_width, self_height);

    // QOIChunks@0(ref_mut(state))[QOIHeader@0.END, EOI]
    left = nt_QOIHeader_0_ipg_end as usize;
    right = EOI as usize;
    if right < left || right > EOI { break '_ipg_alt; }
    let nt_QOIChunks_0_m = QOIChunks(input, begin + left, begin + right, &mut (self_state));
...
fn QOIChunks(input: &[u8], begin: usize, end: usize, a_state: &mut State) -> Option<(usize, usize, QOIChunks)> {
  let EOI: usize = end - begin;
  '_ipg_alt: {
    let mut left: usize = EOI; let mut right: usize = 0;
    let mut self_ipg_start: usize = EOI; let mut self_ipg_end: usize = 0;
    ...
    loop {
      ...
      let nt_QOIChunk_0_m = QOIChunk(input, begin + left, begin + right, a_state);
...
fn QOIChunk(input: &[u8], begin: usize, end: usize, a_state: &mut State) -> Option<(usize, usize, QOIChunk)> {
  let EOI: usize = end - begin;
  '_ipg_alt: {
    let mut left: usize = EOI; let mut right: usize = 0;
    let mut self_ipg_start: usize = EOI; let mut self_ipg_end: usize = 0;
    ...
    right = EOI as usize;
    if right < left || right > EOI { break '_ipg_alt; }
    let nt_Chunk_0_m = Chunk(input, begin + left, begin + right, a_state, self_tagByte);
    ...
    // { chunk = Chunk@0.run }
    let mut self_chunk = nt_Chunk_0.run;
...
fn Chunk(input: &[u8], begin: usize, end: usize, a_state: &mut State, a_tagByte: u8) -> Option<(usize, usize, Chunk)> {
  let EOI: usize = end - begin;
  '_ipg_alt: {
    let mut left: usize = EOI; let mut right: usize = 0;
    let mut self_ipg_start: usize = EOI; let mut self_ipg_end: usize = 0;
    ...
    // { run = RGB(state, r, g, b) }
    let mut self_run = RGB(a_state, self_r, self_g, self_b);
  ...
  '_ipg_alt: {
    ...
    let mut self_run = RGBA(a_state, self_r, self_g, self_b, self_a);
  ...
  '_ipg_alt: {
    ...
    let mut self_run = Index(a_state, self_index);
  ...
  '_ipg_alt: {
    ...
    let mut self_run = Diff(a_state, self_dr, self_dg, self_db);
  ...
  '_ipg_alt: {
    ...
    let mut self_run = Luma(a_state, self_diffGreen, self_drdg, self_dbdg);
  ...
  '_ipg_alt: {
    ...
    let mut self_runLength = (a_tagByte & 63) + 1;

    // { run = Run(state, runLength) }
    let mut self_run = Run(a_state, self_runLength);
