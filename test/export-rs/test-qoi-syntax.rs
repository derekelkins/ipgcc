...
fn QOI(input: &[u8], begin: usize, end: usize) -> Option<(usize, usize, QOI)> {
  ...
  '_ipg_alt: {
    ...
    let self_width = nt_QOIHeader_0.width;

    // { height = QOIHeader@0.height }
    let self_height = nt_QOIHeader_0.height;

    // { channels = QOIHeader@0.channels }
    let self_channels = nt_QOIHeader_0.channels;

    // { colorspace = QOIHeader@0.colorspace }
    let self_colorspace = nt_QOIHeader_0.colorspace;
    ...
    let self_chunks = nt_QOIChunks_0.values;
...
fn QOIHeader(input: &[u8], begin: usize, end: usize) -> Option<(usize, usize, QOIHeader)> {
  ...
  '_ipg_alt: {
    ...
    right = 4 as usize;
    if right < left || right > EOI { break '_ipg_alt; }
    if !&input[begin + left .. begin + right].starts_with("qoif".as_bytes()) { break '_ipg_alt; }
    ...
    let self_width = nt_BE_U32_0.value;
    ...
    right = left + 1;
    if right > EOI { break '_ipg_alt; }
    let self_colorspace = input[begin + left];
    self_ipg_start = self_ipg_start.min(left);
    self_ipg_end = self_ipg_end.max(right);
...
fn QOIChunk(input: &[u8], begin: usize, end: usize) -> Option<(usize, usize, QOIChunk)> {
  ...
  '_ipg_alt: {
    ...
    right = left + 1;
    if right > EOI { break '_ipg_alt; }
    let self_tagByte = input[begin + left];
    self_ipg_start = self_ipg_start.min(left);
    self_ipg_end = self_ipg_end.max(right);
    ...
    // { chunk = Chunk@0.chunk }
    let self_chunk = nt_Chunk_0.chunk;
...
fn Chunk(input: &[u8], begin: usize, end: usize, a_tagByte: u8) -> Option<(usize, usize, Chunk)> {
  ...
  '_ipg_alt: {
    ...
    // { tag = "rgb" }
    let self_tag = "rgb".as_bytes().to_vec();

    // { r = .[0] }
    left = 0 as usize;
    right = left + 1;
    if right > EOI { break '_ipg_alt; }
    let self_r = input[begin + left];
    self_ipg_start = self_ipg_start.min(left);
    self_ipg_end = self_ipg_end.max(right);
    ...
    right = left + 1;
    if right > EOI { break '_ipg_alt; }
    let self_b = input[begin + left];
    self_ipg_start = self_ipg_start.min(left);
    self_ipg_end = self_ipg_end.max(right);

    // { chunk = RGB(r, g, b) }
    let self_chunk = RGB(self_r, self_g, self_b);
  ...
  '_ipg_alt: {
    ...
    // { tag = "rgba" }
    let self_tag = "rgba".as_bytes().to_vec();
    ...
    right = left + 1;
    if right > EOI { break '_ipg_alt; }
    let self_a = input[begin + left];
    self_ipg_start = self_ipg_start.min(left);
    self_ipg_end = self_ipg_end.max(right);

    // { chunk = RGBA(r, g, b, a) }
    let self_chunk = RGBA(self_r, self_g, self_b, self_a);
  ...
  '_ipg_alt: {
    ...
    // { tag = "index" }
    let self_tag = "index".as_bytes().to_vec();

    // { index = tagByte & 63 }
    let self_index = a_tagByte & 63;

    // { chunk = Index(index) }
    let self_chunk = Index(self_index);
  ...
  '_ipg_alt: {
    ...
    // { tag = "diff" }
    let self_tag = "diff".as_bytes().to_vec();

    // { dr = wrapping_sub(tagByte >> 4 & 3, 2) }
    let self_dr = wrapping_sub(a_tagByte >> 4 & 3, 2);

    // { dg = wrapping_sub(tagByte >> 2 & 3, 2) }
    let self_dg = wrapping_sub(a_tagByte >> 2 & 3, 2);

    // { db = wrapping_sub(tagByte & 3, 2) }
    let self_db = wrapping_sub(a_tagByte & 3, 2);

    // { chunk = Diff(dr, dg, db) }
    let self_chunk = Diff(self_dr, self_dg, self_db);
  ...
  '_ipg_alt: {
    ...
    // { tag = "luma" }
    let self_tag = "luma".as_bytes().to_vec();

    // { diffGreen = wrapping_sub(tagByte & 63, 32) }
    let self_diffGreen = wrapping_sub(a_tagByte & 63, 32);
    ...
    let self_drdg = wrapping_sub(nt_U8_0.value >> 4, 8);

    // { dbdg = wrapping_sub(U8@0.value & 15, 8) }
    let self_dbdg = wrapping_sub(nt_U8_0.value & 15, 8);

    // { chunk = Luma(diffGreen, drdg, dbdg) }
    let self_chunk = Luma(self_diffGreen, self_drdg, self_dbdg);
  ...
  '_ipg_alt: {
    ...
    // { tag = "run" }
    let self_tag = "run".as_bytes().to_vec();

    // { run = (tagByte & 63) + 1 }
    let self_run = (a_tagByte & 63) + 1;

    // { chunk = Run(run) }
    let self_chunk = Run(self_run);
...
fn EndMarker(input: &[u8], begin: usize, end: usize) -> Option<(usize, usize, EndMarker)> {
  ...
  '_ipg_alt: {
    ...
    right = 8 as usize;
    if right < left || right > EOI { break '_ipg_alt; }
    if !&input[begin + left .. begin + right].starts_with("\x00\x00\x00\x00\x00\x00\x00\x01".as_bytes()) { break '_ipg_alt; }
    self_ipg_start = self_ipg_start.min(left);
    right = left + 8;
    self_ipg_end = self_ipg_end.max(right);
...
fn U8(input: &[u8], begin: usize, end: usize) -> Option<(usize, usize, U8)> {
  ...
  '_ipg_alt: {
    ...
    right = left + 1;
    if right > EOI { break '_ipg_alt; }
    let self_value = input[begin + left];
    self_ipg_start = self_ipg_start.min(left);
    self_ipg_end = self_ipg_end.max(right);
...
fn BE_U32(input: &[u8], begin: usize, end: usize) -> Option<(usize, usize, BE_U32)> {
  ...
  '_ipg_alt: {
    ...
    right = 4 as usize;
    if right < left || right > EOI { break '_ipg_alt; }
    let self_bs = (&input[begin + left .. begin + right]).to_vec();
    if left != right {
      self_ipg_start = self_ipg_start.min(left);
      self_ipg_end = self_ipg_end.max(right);
    }
    ...
    let self_value = (self_bs[3] as i64) | (self_bs[2] as i64) << 8 | (self_bs[1] as i64) << 16 | (self_bs[0] as i64) << 24;
